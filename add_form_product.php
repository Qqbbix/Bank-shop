...
<?php 
$sql = "SELECT MAX(p_id) as last_id from product";
$result = mysqli_query($conn, $sql); 
$row = mysqli_fetch_array($result); 
$sum = $row['last_id'];
if($row['last_id']== NULL && $row['last_id'] != 90000001){
    $last = 90000001;
}else{
    $last = $sum+1;
}
...
<div class="card bg-light mb-3">
  ...
  <div style ="margin-left:20%; margin-right:20%;">
    <div class="form-group">
            ...
            <input type="text" id = "p_id" name="product_id" class="form-control" value ="<?php echo $last; ?>"    readonly>
     </div>

     <div class="form-group">
            <label for="model" class = "col-form-label">รุ่นสินค้า:</label><br>
            <input type="text" id = "model" name="pro_name" class="form-control">
     </div>
     <div class="form-group">
            <label for="band" class = "col-form-label">ยี่ห้อสินค้า:</label><br>
            <input type="text" id = "band" name="pro_band" class="form-control">
     </div>
     <div class="form-group">
    ...
    <div class="form-group">
            ...
            <input type="text" id = "price" name="pro_price" class="form-control ">
     </div>
     <div class="form-group">
        <label for="exampleFormControlTextarea1">รายละเอียดสินค้า</label>
        <textarea class="form-control" id="exampleFormControlTextarea1" rows="3" wordwrap = on name = "pro_des"></textarea>
  </div>
...
<div class="form-group ">
        <input type="submit" name ="submit" class = "btn btn-success mt-3 btn-lg btn-block" value="เพิ่มสินค้า">
</div>
</div>
  </form>
  </div>
